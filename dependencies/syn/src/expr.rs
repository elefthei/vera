...
pub(crate) mod parsing {
    ...
    #[cfg(feature = "full")]
    pub(crate) struct AllowStruct(pub bool);
    ...
    pub(super) fn parse_with_earlier_boundary_rule(input: ParseStream) -> Result<Expr> {
        ...
        parse_with_earlier_boundary_rule_inner(input, attrs)
    }

    pub(crate) fn parse_with_earlier_boundary_rule_block(input: ParseStream) -> Result<Expr> {
        let attrs = input.call(expr_attrs)?;
        let prefix_binop = crate::verus::parse_prefix_binop(input, &attrs, false)?;
        if let Some(expr) = prefix_binop {
            return Ok(expr);
        }
        if input.peek(Token![proof]) && input.peek2(token::Brace) {
            let token: Token![proof] = input.parse()?;
            let op = crate::op::UnOp::Proof(token);
    ...
    pub(super) fn parse_with_earlier_boundary_rule_inner(
        input: ParseStream,
    ...
    fn parse_binop_rhs(
        ...
        loop {
            ...
            if next > precedence
                || next == precedence && Associativity::of(precedence) == Associativity::Right
            {
    ...
    fn rest_of_path_or_macro_or_struct(
        ...
        if (allow_struct.0 || is_certainly_not_a_block(input)) && input.peek(token::Brace) {
            let expr_struct = expr_struct_helper(input, qself, path)?;
            if allow_struct.0 {
...
pub(crate) mod printing {
    ...
    pub(crate) fn print_expr(expr: &Expr, tokens: &mut TokenStream, mut fixup: FixupContext) {
        ...
        let do_print_expr = |tokens: &mut TokenStream| match expr {
            ...
            Expr::View(e) => e.to_tokens(tokens),
            Expr::GetField(e) => e.to_tokens(tokens),
    ...
    fn print_expr_closure(e: &ExprClosure, tokens: &mut TokenStream, fixup: FixupContext) {
        ...
        e.requires.to_tokens(tokens);
        e.ensures.to_tokens(tokens);
        inner_attrs_to_tokens(&e.inner_attrs, tokens);
