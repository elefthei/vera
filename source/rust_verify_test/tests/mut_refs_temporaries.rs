...
test_verify_one_file_with_options! {
    #[test] temporary_place_ctor_update_tail ["new-mut-ref"] => verus_code! {
        ...
        fn mut_ref_pairs2<'a, 'b, 'c, 'd>(a: &'a mut u64, b: &'b mut u64, c: &'c mut u64, d: &'d mut u64) -> (ret: (Pair<&'a mut u64, &'b mut u64>, Pair<&'c mut u64, &'d mut u64>))
            ensures
                af(mut_ref_current(ret.0.a) == mut_ref_current(a)),
                af(mut_ref_future(ret.0.a) == mut_ref_future(a)),
                af(mut_ref_current(ret.0.b) == mut_ref_current(b)),
                af(mut_ref_future(ret.0.b) == mut_ref_future(b)),
                af(mut_ref_current(ret.1.a) == mut_ref_current(c)),
                af(mut_ref_future(ret.1.a) == mut_ref_future(c)),
                af(mut_ref_current(ret.1.b) == mut_ref_current(d)),
                af(mut_ref_future(ret.1.b) == mut_ref_future(d)),
        {
...
test_verify_one_file_with_options! {
    #[test] temporary_place_in_let_stmt ["new-mut-ref"] => verus_code! {
        broadcast proof fn stronger_resolver_axiom<A, B>(pair: (A, B)) // TODO(new_mut_ref)
            ensures #[trigger] af(has_resolved(pair) ==> has_resolved(pair.0) && has_resolved(pair.1))
        { }
...
test_verify_one_file_with_options! {
    #[test] temporary_place_in_match_with_option ["new-mut-ref"] => verus_code! {
        ...
        broadcast proof fn stronger_resolve_axiom_opt<A>(opt: Option<A>) // TODO(new_mut_ref)
            ensures #[trigger] af(has_resolved(opt) ==> opt is Some ==> has_resolved(opt->Some_0))
        {
        }
