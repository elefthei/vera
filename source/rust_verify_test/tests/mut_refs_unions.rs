...
test_verify_one_file_with_options! {
    #[test] union_with_mut_ref_in_fields ["new-mut-ref"] => verus_code! {
        ...
        fn manually_drop_deref_mut<T>(m: &mut ManuallyDrop<T>) -> (ret: &mut T)
            ensures
                af(done(mut_ref_current(ret) == mut_ref_current(m)@)),
                af(done(mut_ref_future(ret) == mut_ref_future(m)@)),
        {
            m.deref_mut()
        }
...
test_verify_one_file_with_options! {
    #[ignore] #[test] eval_order_union_slice_issue1 ["new-mut-ref"] => verus_code! {
        ...
        #[verifier::external_body]
        fn leak<B: ?Sized>(b: Box<B>) -> (ret: &'static B) ensures af(done(ret == b)) { Box::leak(b) }
