        ...
        mod Mod {
            ...
            impl<T> X<T> {
                ...
                pub fn new(t: T) -> (s: Self)
                  ensures af(verus_builtin::done(s.view() === t))
                {
                    X { t: t }
                }
...
test_verify_one_file_with_options! {
    #[test] loop_invariant_except_break_nonlinear ["exec_allows_no_decreases_clause"] => verus_code! {
        fn integer_square_root(n: u32) -> (result: u32)
            ...
            ensures
                af(done(1 <= result <= n)),
                af(done(1 <= result * result <= n)),
                af(done(n < (result + 1) * (result + 1))),
        {
    ...
    #[test] loop_via_subrange verus_code! {
        ...
        fn test_subrange(src: &Vec<u32>, dst: &mut Vec<u32>, lo: usize, hi: usize)
            requires
                ...
                hi <= old(dst).len(),
            ensures
                af(done(src@.subrange(lo as int, hi as int) == dst@.subrange(lo as int, hi as int))),
        {
            for n in lo..hi
                invariant
                    lo <= hi,
