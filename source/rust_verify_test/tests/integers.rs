...
test_verify_one_file! {
    #[test] test_enum_to_int verus_code! {
        ...
        ensures
            af(done(E::A as int == 0)),
            af(done(E::B as int == 255)),
            af(done(E::C as int == 256)),
            af(done(ret == 0 || ret == 255 || ret == 256)),
            af(done(ret ==  v as int)),
        {
            v as usize
        }
    } => Ok(())
