...
const MUT_REF_ARG_SELF_COMMON: &str = verus_code_str! {
    ...
    impl Value {
        pub fn add1(&mut self)
            ...
            ensures af(done(self.v == old(self).v + 1))
        {
            let Value { v } = *self;
            *self = Value { v: v + 1 };
        }
...
test_verify_one_file! {
    #[test] test_mut_ref_forward verus_code! {
        fn div2(a: &mut u64)
            ensures af(done(*a == *old(a) / 2))
        {
            *a = *a / 2;
        }

        fn test(b: &mut u64)
            ensures af(done(*b == *old(b) / 2))
        {
            div2(b);
        }
    } => Ok(())
...
test_verify_one_file! {
    #[test] test_mut_ref_trigger_0 verus_code! {
        ...
        exec fn add1(a: &mut A, i: usize)
            ensures
                af(done(forall|j: nat| a.index(j) == old(a).index(j) + 1))
        {
            assume(false);
        }
    } => Ok(())
...
test_verify_one_file_with_options! {
    #[test] test_regression_115_mut_ref_pattern_case_2 ["--no-external-by-default"] => code! {
        fn foo(x: &mut bool) -> (u8, u8) {
            ensures(|ret: (u8, u8)| af(done((*x) == ! *old(x))));
