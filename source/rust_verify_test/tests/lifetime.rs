...
test_verify_one_file! {
    #[test] two_phase verus_code! {
        ...
        impl X {
            ...
            #[verifier::external_body]
            fn len(&self) -> (l: u64)
                ensures af(done(l == self.seq().len()))
            {
                unimplemented!();
            }
            ...
            fn push(&mut self, elem: u64)
                ensures
                    af(done(self.seq() == old(self).seq().push(elem)))
            {
                unimplemented!();
            }
        }
    ...
    #[test] two_phase_with_overloaded_compound_assignment_operator verus_code! {
        ...
        impl std::ops::AddAssign<u64> for X {
            ...
            fn add_assign(&mut self, rhs: u64)
                ensures af(done(self.seq() == old(self).seq().push(rhs)))
            {
                unimplemented!();
            }
        }
