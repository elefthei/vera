...
test_verify_one_file! {
    #[test] test_trait_arg verus_code! {
        trait T<A> {
            proof fn f(a: &A) ensures af(done(true));
        }
        struct S;
        impl<B> T<B> for S {
            proof fn f(b: &B) ensures af(done(true)) {  }
        }
    } => Ok(())
}
    ...
    #[test] test_trait_arg4 verus_code! {
        ...
        trait Tr<B: Compare> {
            proof fn stuff(a: B, b: B, c: B) -> (res: (B, B, B))
                requires a.comp(&b), b.comp(&c),
                ensures af(done(res.0.comp(&res.1)));
        }
        ...
        impl<B: Compare> Tr<X<B>> for Y<B> {
            proof fn stuff(a: X<B>, b: X<B>, c: X<B>) -> (res: (X<B>, X<B>, X<B>))
                ensures af(done(res.1.comp(&res.2)))
            {
                return (a, a, b); // FAILS
            }
        }
