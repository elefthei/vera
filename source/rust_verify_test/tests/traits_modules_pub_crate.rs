...
test_verify_one_file! {
    #[test] test_termination_1 verus_code! {
        mod M1 {
            ...
            pub(crate) closed spec fn rec<A: T>(x: &A) {
                x.f();
            }
        }
...
test_verify_one_file! {
    #[test] test_verify_2 verus_code! {
        mod M1 {
            pub(crate) trait T {
                fn f(&self)
                    ensures af(verus_builtin::done(false)); // TRAIT
            }
        }
        mod M2 {
            struct S {}
    ...
    #[test] test_verify_4 verus_code! {
        mod M1 {
            pub(crate) trait T {
                ...
                fn f(&self)
                    ensures af(verus_builtin::done(self.ens())); // TRAIT
            }
        }
        mod M2 {
            struct S {}
    ...
    #[test] test_verify_6 verus_code! {
        mod M1 {
            pub(crate) trait T<A> {
                ...
                fn f(&self, a: &A) -> (ra: A)
                    requires self.req(*a),
                    ensures af(verus_builtin::done(self.ens(*a, ra))); // TRAIT
            }
        }
        ...
        mod M6 {
            pub(crate) fn p<A, Z: crate::M1::T<A>>(a: &A, z: &Z) -> (rz: A)
                requires z.req(*a)
                ensures af(verus_builtin::done(z.ens(*a, rz)))
            {
                z.f(a)
            }
        }
    ...
    #[test] test_generic_1_private verus_code! {
        ...
        mod M4 {
            ...
            proof fn test() -> (b: bool)
                ensures af(verus_builtin::done(b)) // FAILS
            {
                let i: u8 = 10;
...
test_verify_one_file! {
    #[test] test_generic_1_public_ok verus_code! {
        ...
        mod M4 {
            #[allow(unused_imports)] use crate::M1::T;
            proof fn test() -> (b: bool)
                ensures af(verus_builtin::done(b))
            {
                let i: u8 = 10;
                let s = crate::M2::S(true, i);
                let b: bool = s.apple((i, 20));
...
test_verify_one_file! {
    #[test] test_generic_3 verus_code! {
        mod M1 {
            pub(crate) trait T {
                ...
                fn banana(&self)
                    ...
                    ensures af(verus_builtin::done(true));
            }
        }
        ...
        mod M3 {
            pub(crate) fn f1<A: crate::M1::T>(a: &A)
        ...
        mod M1 {
            pub(crate) trait T {
                ...
                fn f<'a>(&'a self, x: &'a Self, b: bool) -> (r: &'a Self)
                    ensures
                        af(verus_builtin::done(b ==> r === self)),
                        af(verus_builtin::done(!b ==> r === x));
            }
        ...
        mod M1 {
            pub(crate) trait T {
                fn f<'a>(&'a self, x: &'a Self, b: bool) -> (r: &'a Self)
                    ensures
                        af(verus_builtin::done(b ==> r === self)),
                        af(verus_builtin::done(!b ==> r === x)); // TRAIT
            }
        }
