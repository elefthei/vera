...
test_verify_one_file! {
    #[test] trait_assoc_type_bound_generic verus_code! {
        ...
        trait B {
            ...
            proof fn foo(t: Self::T)
                requires t.a(),
                ensures af(done(Self::ens(t)));
        }

        proof fn bar<BB: B>(t: BB::T)
            requires t.a(),
            ensures af(done(BB::ens(t))),
        {
            BB::foo(t);
        }
    } => Ok(())
    ...
    #[test] trait_typ_equality1 verus_code! {
        ...
        impl T for bool {
            ...
            fn g(x: &Self::X, y: &Self::Y) -> (r: Self::Y)
                ensures af(done(r == *x as u16 + *y / 2))
            {
                *x as u16 + *y / 2
            }
        }
...
test_verify_one_file! {
    #[test] trait_typ_equality_broadcast verus_code! {
        ...
        broadcast proof fn b<A: T<X = u8>>(x: A::X, a: A)
            ensures af(done(#[trigger] q(a, x) && p(x)))
        {
            p_u8(x);
        }
...
test_verify_one_file! {
    #[test] trait_typ_equality_dispatch_proof verus_code! {
        ...
        impl<A: U<X = u8>> T for A { proof fn f() -> (r: int) ensures af(done(r == 100)) { 100 } }
        impl T for bool { proof fn f() -> (r: int) ensures af(done(r == 200)) { 200 } }
