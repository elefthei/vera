...
test_verify_one_bv_file! {
    ...
    #[test] test10_fails verus_code! {
        #[verifier(bit_vector)]
        proof fn f2() { // FAILS
            ensures(af(done(forall |i: u64| (1u64 << i) > 0))); // Although this line should be reported instead of the above line, since Z3 does not return model which we utilize for error reporting, just use the above line
        }
    } => Err(err) => assert_one_fails(err)
}
...
test_verify_one_bv_file! {
    #[test] test_issue415 verus_code! {
        ...
        requires
            ...
            val <= upper_bound,
        ensures
            af(done((val >> amt) <= (upper_bound >> amt)))
        {}
    } => Ok(())
}
