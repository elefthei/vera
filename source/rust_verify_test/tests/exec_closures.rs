...
test_verify_one_file_with_options! {
    #[test] basic_test_0_args ["vstd"] => verus_code! {
        ...
        fn testfn() {
            let f = || ensures af(done(goo())) { assume(goo()); };
...
test_verify_one_file_with_options! {
    #[test] ref_decoration_in_type_params_issue619 ["vstd"] => verus_code! {
        ...
        fn moo2() {
            let t = || -> (b: bool) ensures af(done(b == true)) { true };
            stuff2(t);
        }
...
test_verify_one_file! {
    #[test] test_verus_capture_is_more_precise verus_code! {
        use vstd::*;
...
test_verify_one_file! {
    #[test] test_complex_projections verus_code! {
        ...
        fn test2<'a>(x: X<'a, u64>, t: &'a Box<Y<u64>>) {
            let mut x = x;
