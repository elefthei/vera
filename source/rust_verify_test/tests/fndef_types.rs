    ...
    #[test] recursion18_via_trait_function_with_extended_ensures verus_code! {
        ...
        impl Tr for X {
            fn stuff(&self) -> (res: bool)
                ensures af(done(call_ensures(X::stuff, (&self,), res)))
            {
                true
            }
        }
...
test_verify_one_file! {
    #[test] recursion19_via_trait_function_ensures verus_code! {
        trait Tr {
            fn stuff(&self) -> (res: bool)
                ensures af(done(call_ensures(Self::stuff, (&self,), res)));
        }
    } => Err(err) => assert_vir_error_msg(err, "cyclic dependency in the requires/ensures of function")
}
        ...
        trait VEq : VPartialEq {
            ...
            proof fn fns_correct(a: &Self, b: &Self)
                ensures
                    ...
                    af(done(call_ensures(Self::eq, (a, b), true) ==> a.rel(b))),
                    af(done(call_ensures(Self::eq, (a, b), false) ==> !a.rel(b))),

                    af(done(call_ensures(Self::ne, (a, b), true) ==> !a.rel(b))),
                    af(done(call_ensures(Self::ne, (a, b), false) ==> a.rel(b)));
        }
...
test_verify_one_file! {
    #[test] trait_method_use_emits_req_ens_axioms_for_impls_fail verus_code! {
        ...
        trait VEq : VPartialEq {
            ...
            proof fn fns_correct(a: &Self, b: &Self)
                ensures
                    af(done(call_ensures(Self::ne, (a, b), true) ==> !a.rel(b))); // FAILS
        }
...
test_verify_one_file! {
    #[test] trait_default_method_call_ensures_with_default_spec_fn verus_code! {
        trait Tr {
            ...
            fn hello(i: u64, j: u64)
                ensures af(done(Self::stuff(i, j)))
            {
                assume(false);
            }
        }
        ...
        impl<A> Tr for Y<A> {
            fn hello(i: u64, j: u64)
                ensures af(done(i >= 5)), // and implied stuff()
            {
                assume(false);
            }
        }
    ...
    #[test] bound_vars_issue1005 verus_code! {
        ...
        fn vec_map<T, U>(v: &Vec<T>, f: impl Fn(&T) -> U) -> (result: Vec<U>)
            ...
            ensures
                ...
                af(done(forall |i| 0 <= i < v.len() ==> call_ensures(f, (&v[i],), #[trigger] result[i])))
        {
            assume(false);
            Vec::new()
        }
    ...
    #[test] unrecognized_trait_impl_issue1332 verus_code! {
        ...
        impl Foo {
            proof fn lemma_clone()
                ensures
                    af(done(forall |a: Self, b: Self| call_ensures(Clone::clone, (&a,), b) ==> a == b)),
            { assume(false); }
        }
    ...
    #[test] clone_assign_type_param_trait_function_to_variable verus_code! {
        ...
        impl<T: Clone> Clone for X<T> {
            fn clone(&self) -> (s: Self)
                ensures
                    af(done(call_ensures(T::clone, (&self.t,), s.t)))
            {
                let t_clone = T::clone;
    ...
    #[test] generic_conditions_for_normal_call_traits verus_code! {
        trait Tr : Sized {
            ...
            fn foo(&self) -> (a: Self)
                ...
                ensures af(done(self.foo_ens(a)));
        }
