...
impl CounterSharedState {
    ...
    pub fn new(
        ...
        ensures
            af(done(result.wf())),
            af(done(result.get_oneshot_id(0) == oneshot0_inv_half.id())),
            af(done(result.get_oneshot_id(1) == oneshot1_inv_half.id())),
    {
...
pub fn thread_routine(
    ...
    ensures
        af(done(return_permission@.id() == shared_state.get_oneshot_id(which_thread))),
        af(done(return_permission@@ is Complete)),
{
...
pub fn count_to_two() -> (result: Result<u32, ()>)
    ensures
        af(done(result is Ok ==> result.unwrap() == 2)),
{
