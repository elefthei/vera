...
// The `af(done(...))` syntax makes the temporal intent explicit.
//
// Temporal VCGen:
//   - `ensures af(done(...))` declares the temporal postcondition (sugar for au(true, ...))
...
impl Queue {
    ...
    fn is_empty(&self) -> (b: bool)
        ensures af(done(b == (self.view().len() == 0))),
    {
        self.data.len() == 0
    }
}
...
///
/// The temporal postcondition `ensures af(done(q.view().len() == 0))` states that
