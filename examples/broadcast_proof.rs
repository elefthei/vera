  ...
  mod multiple_broadcast_proof {
    ...
    impl Multiple {
      ...
      pub broadcast proof fn lemma_add_aligned(p: Self, v: Self)
        ...
        ensures
          af(done((#[trigger] p.add(v)).aligned())),
          af(done(p.add(v).modulo == lib::same_or_arbitrary(p.modulo, v.modulo))),
      {
      ...
      pub broadcast proof fn lemma_mul_aligned(p: Self, v: Self)
        requires
          p.aligned(), v.aligned(), p.modulo == v.modulo,
        ensures
          af(done((#[trigger] p.mul(v)).aligned())),
          af(done(p.mul(v).modulo == lib::same_or_arbitrary(p.modulo, v.modulo))),
      {
        // TODO
        admit();
      }
