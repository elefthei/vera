...
proof fn hoisted_forall(i: int, j: int)
    ensures
        af(done(is_distinct(i, j) ==> is_distinct(j, i))),
{
}
...
proof fn test_even_f()
    ensures
        af(done(forall|i: int| is_even(i) ==> f(i))), // FAILS because we don't call the lemma
{
}
...
proof fn test_even_f()
    ensures
        af(done(forall|i: int| is_even(i) ==> f(i))),
{
